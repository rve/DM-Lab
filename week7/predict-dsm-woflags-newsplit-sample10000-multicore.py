...
acc_ada= round(ada_boost.score(X_train, Y_train) * 100, 2)
ts_acc_ada = round(ada_boost.score(X_validate, Y_validate) * 100, 2)

# XGBoost
xgb_boost = xgb.XGBRegressor(seed=1850)
xgb_boost.fit(X_train, Y_train) 
acc_xgb= round(xgb_boost.score(X_train, Y_train) * 100, 2)
ts_acc_xgb = round(xgb_boost.score(X_validate, Y_validate) * 100, 2)
...
models = pd.DataFrame({
    ...
    'Train Set Score': [ts_acc_svc, ts_acc_2nn, ts_acc_log, 
              ...
              ts_acc_sgd, ts_acc_linear_svc, ts_acc_decision_tree, ts_acc_xgb, ts_acc_ada],
    'Validate Set Score': [acc_svc, acc_2nn, acc_log,
              acc_random_forest, acc_gaussian, acc_perceptron, 
              acc_sgd, acc_linear_svc, acc_decision_tree, acc_xgb, acc_ada]
    })
